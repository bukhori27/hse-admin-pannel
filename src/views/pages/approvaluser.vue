    ...
    <div class="container m-t-60">
      <b-row class="justify-content-center">
        <div class="col-md-12 col-sm-12 col-lg-12 col-xl-12 col-xs-11">
          <div class="col-md-12"><h1 class="tc m-b-50">Approval User</h1> </div>
          <!-- step1 -->
  ...
  export default {
    ...
    methods: {
      ...
      approve (id) {
        ...
        var del = confirm('Apakah anda yakin akan approve lembaga masyarakat ini?')
        if (del) {
          axios.post(url.url_app + 'approval_user_approve', parameter, config).then(function (response) {
            if (response.data.resultCode == 'OK') {
              self.listUser(1)
            } else {
            }
