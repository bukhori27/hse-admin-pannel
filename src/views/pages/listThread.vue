    ...
    <div class="container m-t-60">
      <b-row class="justify-content-center" v-if="listing.length > 0">
        <div class="col-md-12 col-sm-12 col-lg-12 col-xl-12 col-xs-11">
          <div class="col-md-12"><h1 class="tc m-b-50">List Thread</h1> </div>
          <!-- step1 -->
  ...
  export default {
    ...
    methods: {
      ...
      listUser (page) {
        ...
        axios.post(url.url_app + 'forum_thread_filter_admin', parameter, config).then(function (response) {
          if (response.data.resultCode == 'OK') {
            let listings = response.data.forum_thread_list
            for (let i = 0; i < listings.length; i++) {
              let a = {
                id: listings[i].id,
                nama: listings[i].nama,
                author: listings[i].nama_user,
      ...
      deleted(id) {
        ...
        var del = confirm('Apakah anda yakin akan menghapus thread ini?')
        if (del) {
          axios.post(url.url_app + 'forum_thread_delete', parameter, config).then(function (response) {
            if (response.data.resultCode == 'OK') {
              self.listUser(1)
            } else {
            }
